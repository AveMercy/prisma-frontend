import { useState } from 'react';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { assignmentsApi, type Assignment } from '@/api/assignments';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { useAuthStore } from '@/store/authStore';
import { Loader2, Send, CheckCircle, ExternalLink, Clock, AlertCircle } from 'lucide-react';

interface Props {
    assignment: Assignment;
}

export default function AssignmentView({ assignment }: Props) {
    const { user } = useAuthStore();
    const queryClient = useQueryClient();
    const [solutionUrl, setSolutionUrl] = useState('');
    const [showSubmit, setShowSubmit] = useState(false);

    const submission = assignment.submissions?.[0];
    const isTeacher = user?.role === 'teacher' || user?.role === 'admin';
    const isExpired = assignment.dueDate && new Date(assignment.dueDate) < new Date();

    const submitMutation = useMutation({
        mutationFn: () => assignmentsApi.submit(assignment.id, solutionUrl),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['assignments', assignment.lectureId] });
            setShowSubmit(false);
        },
    });

    return (
        <Card className={submission ? 'border-emerald-500/30' : ''}>
            <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                    <div>
                        <h3 className="font-semibold flex items-center gap-2">
                            {assignment.title}
                            {submission && (
                                <Badge variant="default" className="text-xs bg-emerald-500">
                                    <CheckCircle className="h-3 w-3 mr-1" />
                                    Сдано
                                </Badge>
                            )}
                        </h3>
                        {assignment.taskDescription && (
                            <p className="text-sm text-muted-foreground mt-1">{assignment.taskDescription}</p>
                        )}
                    </div>
                    {assignment.dueDate && (
                        <div className={`flex items-center gap-1 text-xs ${isExpired ? 'text-red-500' : 'text-muted-foreground'}`}>
                            {isExpired ? <AlertCircle className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                            {new Date(assignment.dueDate).toLocaleDateString('ru-RU')}
                        </div>
                    )}
                </div>

                {/* Сдача студента */}
                {submission && (
                    <div className="mt-3 p-3 rounded-lg bg-muted">
                        <div className="flex items-center justify-between">
                            <a
                                href={submission.solutionUrl || '#'}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-sm text-primary hover:underline flex items-center gap-1"
                            >
                                <ExternalLink className="h-3 w-3" />
                                {submission.solutionUrl || 'Ссылка не указана'}
                            </a>
                            {submission.grade !== null && (
                                <Badge variant="secondary">
                                    Оценка: {submission.grade}
                                </Badge>
                            )}
                        </div>
                        {submission.feedback && (
                            <p className="text-sm text-muted-foreground mt-2">
                                💬 {submission.feedback}
                            </p>
                        )}
                    </div>
                )}

                {/* Кнопка сдачи (для студента без сдачи) */}
                {!isTeacher && !submission && (
                    <>
                        {!showSubmit ? (
                            <Button
                                variant="outline"
                                size="sm"
                                className="mt-3"
                                onClick={() => setShowSubmit(true)}
                            >
                                <Send className="h-4 w-4 mr-2" />
                                Сдать задание
                            </Button>
                        ) : (
                            <div className="mt-3 space-y-3 p-3 rounded-lg bg-muted">
                                <div className="space-y-2">
                                    <Label>Ссылка на решение (GitHub)</Label>
                                    <Input
                                        value={solutionUrl}
                                        onChange={(e) => setSolutionUrl(e.target.value)}
                                        placeholder="https://github.com/user/repo"
                                    />
                                </div>
                                <div className="flex gap-2">
                                    <Button
                                        size="sm"
                                        onClick={() => submitMutation.mutate()}
                                        disabled={!solutionUrl || submitMutation.isPending}
                                    >
                                        {submitMutation.isPending ? (
                                            <Loader2 className="h-4 w-4 animate-spin" />
                                        ) : (
                                            'Отправить'
                                        )}
                                    </Button>
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        onClick={() => setShowSubmit(false)}
                                    >
                                        Отмена
                                    </Button>
                                </div>
                            </div>
                        )}
                    </>
                )}
            </CardContent>
        </Card>
    );
}